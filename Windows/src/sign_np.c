...
NPUMDIMG_HEADER* forge_npumdimg(int iso_size, int iso_blocks, int block_basis, char *content_id, int np_flags, u8 *version_key, u8 *header_key, u8 *data_key)
{
	...
	u8 *npumdimg_sha1_inbuf = (u8 *) malloc(0xD8 + 0x4);
	u8 *npumdimg_sha1_outbuf = (u8 *) malloc(0x14);
	...
	u8 *npumdimg_sign_buf_in = (u8 *) malloc(0x34);
	u8 *npumdimg_sign_buf_out = (u8 *) malloc(0x28);
	memset(npumdimg_sign_buf_in, 0, 0x34);
	memset(npumdimg_sign_buf_out, 0, 0x28);

	// Create ECDSA key pair.
	u8 *npumdimg_keypair = (u8 *) malloc(0x3C);
	...
	u8 *npumdimg_private_key_enc = (u8 *) malloc(0x20);
	...
	u8 *test_npumdimg_sign = (u8 *) malloc(0x64);
...
int write_pbp(FILE *f, char *iso_name, char *content_id, int np_flags, u8 *startdat_buf, int startdat_size, u8 *pgd_buf, int pgd_size)
{
	...
	*(u32 *)(data_psp_buf + 0x590) = se32(np_flags);
	...
	u8 *data_psp_sha1_outbuf = (u8 *) malloc (0x14);
	...
	*(u32 *)data_psp_sha1_inbuf = param_sfo_size + 0x30;
	...
	// Verify the generated ECDSA signature.
	u8 *test_data_psp_sign = (u8 *) malloc(0x64);
...
{
	printf("************************************************************\n\n");
	printf("sign_np v1.0.5 - Convert PSP ISOs to signed PSN PBPs.\n");
...
int main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[arg_offset + 1], "-elf") && (argc > (arg_offset + 4)))
	{
		...
		int elf_size = (int)_ftelli64(elf);
	...
	else if (!strcmp(argv[arg_offset + 1], "-pbp") && (argc > (arg_offset + 5)))  // EBOOT signing mode.
	{
		...
		long long iso_size = _ftelli64(iso);
		_fseeki64(iso, 0, SEEK_SET);
		...
		if (opnssmp_name)
		{
			...
			int opnssmp_size = (int)_ftelli64(opnssmp);
		...
		if (startdat_name)
		{
			...
			int png_size = (int)_ftelli64(png);
		...
		u8 *table_buf = malloc((size_t)table_size);
		memset(table_buf, 0, (size_t)table_size);
		fwrite(table_buf, (size_t)table_size, 1, pbp);
		...
		for(i = 0; i < iso_blocks; i++)
		{
			...
			if ((_ftelli64(iso) + block_size) > iso_size)
			{
				long long remaining = iso_size - _ftelli64(iso);
				fread(iso_buf, (size_t)remaining, 1, iso);
				wsize = (int)remaining;
			}
			else
			{
			...
			// Set table entry.
			*(u32*)(tb + 0x10) = (u32)iso_offset;
			*(u32*)(tb + 0x14) = wsize;
			*(u32*)(tb + 0x18) = 0;
			*(u32*)(tb + 0x1C) = 0;

			// Encrypt block.
			sceDrmBBCipherInit(&ckey, 1, 2, header_key, version_key, (u32)(iso_offset >> 4));
		...
		// Generate data key.
		sceDrmBBMacInit(&mkey, 3);
		sceDrmBBMacUpdate(&mkey, table_buf, (int)table_size);
		...
		_fseeki64(pbp, np_offset, SEEK_SET);
		fwrite(npumdimg, np_size, 1, pbp);
		_fseeki64(pbp, table_offset, SEEK_SET);
		fwrite(table_buf, (size_t)table_size, 1, pbp);
