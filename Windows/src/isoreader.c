...
__declspec(align(64))
static char g_sector_buffer[SECTOR_SIZE];
...
static _inline u32 isoPos2LBA(u32 pos)
{
	return pos / SECTOR_SIZE;
}

static _inline u32 isoLBA2Pos(u32 lba, int offset)
{
	return lba * SECTOR_SIZE + offset;
}

static _inline u32 isoPos2OffsetInSector(u32 pos)
{
	return pos & (SECTOR_SIZE - 1);
}

static _inline u32 isoPos2RestSize(u32 pos)
{
...
static int readSectorCompressed(int sector, void *addr)
{
	...
	ret = gzip_decompress(addr, SECTOR_SIZE, (void*)((uintptr_t)g_ciso_dec_buf + offset - g_ciso_dec_buf_offset), size);
...
int isoOpen(const char *path)
{
	...
	if (g_is_compressed) {
		g_total_sectors = (u32)(g_ciso_h.total_bytes / g_ciso_h.block_size);
		g_CISO_cur_idx = -1;
...
int isoRead(void *buffer, u32 lba, int offset, u32 size)
{
	...
	while(remaining > 0) {
		...
		memcpy((void*)((uintptr_t)buffer+copied), g_sector_buffer+isoPos2OffsetInSector(pos), re);
		remaining -= re;
		pos += re;
		copied += re;
	}
